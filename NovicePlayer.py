...
# Imports for the evaluation function
import torch
from net import DQN
...
class HistoryCell:
  ...
  def most_prob_range(self):
    max_cell = max(self.prob_win_frequency_cell)
    pos_range = list()
    for i in range(0, 10):
      if self.prob_win_frequency_cell[i] == max_cell:
        pos_range.append(i/10.0)
    min_prob = min(pos_range)
    max_prob = max(pos_range)
    return [min_prob, max_prob + 0.1]
...
class SequenceActionTree: 
  def __init__(self):
    ...
    # Initialize neural network for evaluation function
    N_FEAT = 6
    N_ACTIONS = 1
    N_HIDDEN = int((N_FEAT + N_ACTIONS) / 2)
    N_LAYERS = 2
    self.net = DQN(N_FEAT, N_HIDDEN, N_ACTIONS, N_LAYERS)
  ...
  def declare_action(self, hole_card, current_node, depth, opp_model, round_state, cut_off_nodes, prob_win):
    ...
    for node in cut_off_nodes:
      if node.name[len(node.name) - 1] != "f":
        ...
        feat_vector = [our_prob_win, opp_pro_win, our_number_raise_at_street,
